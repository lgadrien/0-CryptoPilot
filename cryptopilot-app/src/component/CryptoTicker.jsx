...
function CryptoTicker() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        setLoading(true);
        // Utilisation de l'API CoinGecko directement (gratuite, pas d'auth requise)
  ...
  if (error) {
    return (
      <div
        className="w-full text-center py-3 text-red-400 font-sans bg-gray-100 dark:bg-[#0B0D12] transition-colors duration-300"
      >
        {error}
      </div>
    );
  ...
  if (loading || cryptos.length === 0) {
    return (
      <div
        className="w-full text-center py-3 text-gray-500 dark:text-gray-500 font-sans bg-gray-100 dark:bg-[#0B0D12] transition-colors duration-300"
      >
